...
class DeduplicationEngine:
    ...
    def add_gedcom_data(self, individuals: Dict[str, Individual], 
                       families: Dict[str, Family], source_file: str):
    ...
    def detect_duplicates(self) -> List[DuplicateMatch]:
        ...
        for i in range(len(individual_ids)):
            for j in range(i + 1, len(individual_ids)):
                ...
                if match and match.similarity_score >= 0.6:  # Threshold for potential match
    ...
    def _calculate_similarity(self, ind1: Individual, ind2: Individual,
                            id1: str, id2: str) -> Optional[DuplicateMatch]:
        ...
        if (ind1.birth_year and ind2.birth_year and 
            abs(ind1.birth_year - ind2.birth_year) <= 1):
    ...
    def _calculate_location_similarity(self, ind1: Individual, ind2: Individual) -> float:
    ...
    def create_merge_actions(self, confirmed_matches: List[DuplicateMatch]) -> List[MergeAction]:
    ...
    def _merge_individuals(self, primary: Individual, duplicate: Individual, 
                          merged_id: str) -> Individual:
        ...
        if duplicate.given_names and len(duplicate.given_names) > len(primary.given_names or ""):
...
def deduplicate_gedcom_files(gedcom_files: List[str], 
                           auto_merge_threshold: float = 0.95) -> Tuple[Dict[str, Individual], Dict[str, Family], List[DuplicateMatch]]:
    """
    ...
    auto_merge = [dup for dup in duplicates if dup.similarity_score >= auto_merge_threshold]
    remaining_duplicates = [dup for dup in duplicates if dup.similarity_score < auto_merge_threshold]
