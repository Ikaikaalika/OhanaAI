...
from .gedcom_parser import parse_gedcom_file, Individual, Family
...
class OhanaAIGUI:
    ...
    def _create_menu_bar(self):
        ...
        file_menu.add_command(label="Load GEDCOM Files...", command=self._load_gedcom_files)
        file_menu.add_separator()
        file_menu.add_command(label="Save Predictions...", command=self._save_predictions)
        ...
        dedup_menu.add_command(label="Detect Duplicates", command=self._detect_duplicates)
        dedup_menu.add_command(label="Review Duplicates", command=self._review_duplicates)
    ...
    def _setup_left_panel(self, parent):
        ...
        ttk.Button(file_buttons, text="Add Files", command=self._load_gedcom_files).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(file_buttons, text="Remove", command=self._remove_selected_file).pack(side=tk.LEFT)
        
        # Model section
        model_frame = ttk.LabelFrame(parent, text="Model Operations", padding=5)
        model_frame.pack(fill=tk.X, pady=(0, 5))
        
        ttk.Button(model_frame, text="Train Model", command=self._train_model, width=20).pack(pady=2, fill=tk.X)
        ttk.Button(model_frame, text="Load Model", command=self._load_model, width=20).pack(pady=2, fill=tk.X)
        ttk.Button(model_frame, text="Run Predictions", command=self._run_predictions, width=20).pack(pady=2, fill=tk.X)
        
        # Deduplication section
        dedup_frame = ttk.LabelFrame(parent, text="Deduplication", padding=5)
        dedup_frame.pack(fill=tk.X, pady=(0, 5))
        
        ttk.Button(dedup_frame, text="Detect Duplicates", command=self._detect_duplicates, width=20).pack(pady=2, fill=tk.X)
        ttk.Button(dedup_frame, text="Review Duplicates", command=self._review_duplicates, width=20).pack(pady=2, fill=tk.X)
        ...
        self.confidence_var = tk.DoubleVar(value=self.config['gui']['confidence_threshold'])
    ...
    def _setup_predictions_tab(self, parent):
        ...
        filter_scale = ttk.Scale(filter_frame, from_=0.0, to=1.0, 
                               variable=self.filter_var, orient=tk.HORIZONTAL, length=200)
        ...
        self.pred_tree = ttk.Treeview(table_frame, columns=columns, show='headings', height=15)
        ...
        # Scrollbars
        v_scroll = ttk.Scrollbar(table_frame, orient=tk.VERTICAL, command=self.pred_tree.yview)
        h_scroll = ttk.Scrollbar(table_frame, orient=tk.HORIZONTAL, command=self.pred_tree.xview)
        self.pred_tree.configure(yscrollcommand=v_scroll.set, xscrollcommand=h_scroll.set)
    ...
    def _setup_visualization_tab(self, parent):
        ...
        ttk.Button(controls_frame, text="Show Family Tree", command=self._show_family_tree).pack(side=tk.LEFT, padx=5)
        ttk.Button(controls_frame, text="Show Statistics", command=self._show_statistics).pack(side=tk.LEFT, padx=5)
    ...
    def _setup_duplicates_tab(self, parent):
        ...
        self.dup_tree = ttk.Treeview(parent, columns=dup_columns, show='headings', height=15)
        ...
        ttk.Button(dup_controls, text="Merge Selected", command=self._merge_selected_duplicate).pack(side=tk.LEFT, padx=5)
        ttk.Button(dup_controls, text="Ignore", command=self._ignore_duplicate).pack(side=tk.LEFT, padx=5)
    ...
    def _setup_log_tab(self, parent):
        ...
        # Clear button
        ttk.Button(parent, text="Clear Logs", command=lambda: self.log_text.delete(1.0, tk.END)).pack(pady=5)
    ...
    def _train_model(self):
        ...
        if not self.individuals:
            messagebox.showwarning("Warning", "No data loaded. Please load GEDCOM files first.")
            return
    ...
    def _review_duplicates(self):
        ...
        if not self.duplicates:
            messagebox.showinfo("Info", "No duplicates detected. Run duplicate detection first.")
            return
        ...
        try:
            while True:
                ...
                if result_type == 'log':
                    ...
                    self._update_status(f"Parsed {len(self.individuals)} individuals, {len(self.families)} families")
                    ...
                    self._update_status(f"Generated {len(self.predictions)} predictions")
                    self.progress_bar.stop()
                
                elif result_type == 'duplicates_detected':
                    self.dedup_engine, self.duplicates = result_data
                    self._populate_duplicates_table()
                    self._update_status(f"Found {len(self.duplicates)} potential duplicates")
                    self.progress_bar.stop()
    ...
    def _populate_predictions_table(self):
        ...
        for pred in self.predictions:
            if pred.confidence_score >= self.filter_var.get():
                age_diff = f"{pred.age_difference} years" if pred.age_difference else "Unknown"
    ...
    def _populate_duplicates_table(self):
        ...
        for dup in self.duplicates:
            ind1_name = self.individuals.get(dup.individual1_id, type('', (), {'full_name': 'Unknown'})).full_name
            ind2_name = self.individuals.get(dup.individual2_id, type('', (), {'full_name': 'Unknown'})).full_name
    ...
    def _show_statistics(self):
        ...
        # Birth years
        birth_years = [ind.birth_year for ind in self.individuals.values() if ind.birth_year]
        if birth_years:
    ...
    def _save_predictions(self):
        ...
        if filename and self.predictor:
            self.predictor.export_predictions_csv(self.predictions, os.path.basename(filename))
    ...
    def _export_gedcom(self):
        ...
        if filename and self.predictor:
            self.predictor.export_predictions_gedcom(self.predictions, os.path.basename(filename))
