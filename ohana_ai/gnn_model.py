...
class GraphAttentionLayer(nn.Module):
    ...
    def __call__(
        self, x: mx.array, edge_index: mx.array, edge_types: mx.array
    ) -> mx.array:
        """
        ...
        if edge_index.shape[1] == 0:
            ...
            return self.layer_norm(
                transformed + x if x.shape[-1] == self.out_features else transformed
            )
        ...
        edge_embeds = mx.broadcast_to(
            edge_embeds, (edge_embeds.shape[0], self.num_heads, self.head_dim)
        )
        ...
        attention_scores = mx.sum(
            src_queries * modified_keys, axis=-1
        )  # [num_edges, num_heads]
        attention_scores = attention_scores / math.sqrt(self.head_dim)

        # Apply softmax per destination node
        attention_weights = self._softmax_per_node(
            attention_scores, dst_nodes, num_nodes
        )
        ...
        # Aggregate messages
        attention_weights = mx.expand_dims(
            attention_weights, axis=-1
        )  # [num_edges, num_heads, 1]
    ...
    def _softmax_per_node(
        self, scores: mx.array, dst_nodes: mx.array, num_nodes: int
    ) -> mx.array:
    ...
    def _aggregate_messages(
        self, messages: mx.array, dst_nodes: mx.array, num_nodes: int
    ) -> mx.array:
        """Aggregate messages per destination node."""
        # Initialize output
...
class OhanaAIModel(nn.Module):
    ...
    def predict_parents(
        self, node_embeddings: mx.array, candidate_pairs: mx.array
    ) -> mx.array:
        """
    ...
    def __call__(
        ...
        edge_index: mx.array,
        edge_types: mx.array,
        candidate_pairs: Optional[mx.array] = None,
    ) -> mx.array:
        """
        Full forward pass.
...
class ContrastiveLoss(nn.Module):
    ...
    def __call__(
        self, positive_scores: mx.array, negative_scores: mx.array
    ) -> mx.array:
        """
