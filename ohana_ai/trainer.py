...
from .gnn_model import OhanaAIModel, ContrastiveLoss, create_model
...
class OhanaAITrainer:
    ...
    def prepare_data(self, individuals: Dict[str, Individual], 
        ...
        logger.info(f"Training data prepared: {len(self.train_indices)} train, {len(self.val_indices)} val samples")
...
def train_model(gedcom_files: List[str], config_path: str = "config.yaml") -> OhanaAITrainer:
