...
class GraphBuilder:
    ...
    def build_graph(self, individuals: Dict[str, Individual], 
                   families: Dict[str, Family]) -> GraphData:
        """Build graph from individuals and families."""
        logger.info(f"Building graph from {len(individuals)} individuals and {len(families)} families")
        ...
        # Build edges
        edge_index, edge_types = self._build_edges(valid_individuals, families, id_to_idx)
        ...
        logger.info(f"Built graph with {graph_data.num_nodes} nodes and {graph_data.num_edges} edges")
        return graph_data
    
    def _filter_valid_individuals(self, individuals: Dict[str, Individual]) -> Dict[str, Individual]:
        ...
        for ind_id, individual in individuals.items():
            ...
            if (individual.full_name.strip() or 
                individual.birth_year is not None):
                valid[ind_id] = individual
        return valid
    
    def _build_node_features(self, individuals: Dict[str, Individual], 
    ...
    def _individual_to_features(self, individual: Individual) -> np.ndarray:
        ...
        if individual.birth_year:
            normalized_year = (individual.birth_year - self.min_year) / (self.max_year - self.min_year)
            features[idx] = max(0.0, min(1.0, normalized_year))
        idx += 1
        
        # Death year (normalized) 
        if individual.death_year:
            normalized_year = (individual.death_year - self.min_year) / (self.max_year - self.min_year)
            features[idx] = max(0.0, min(1.0, normalized_year))
        idx += 1
        ...
        if full_name and idx < self.feature_dim:
            ...
            for i, (char, count) in enumerate(common_chars[:min(10, self.feature_dim - idx)]):
                features[idx + i] = count / len(full_name)
        
        return features
    
    def _build_edges(self, individuals: Dict[str, Individual], 
                    families: Dict[str, Family], 
                    id_to_idx: Dict[str, int]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for family in families.values():
            ...
            if (family.husband_id and family.wife_id and 
                family.husband_id in id_to_idx and family.wife_id in id_to_idx):
    ...
    def _create_missing_parents_mask(self, individuals: Dict[str, Individual],
                                   families: Dict[str, Family],
                                   node_ids: List[str]) -> np.ndarray:
    ...
    def create_training_pairs(self, graph_data: GraphData, 
                            individuals: Dict[str, Individual],
                            families: Dict[str, Family]) -> Tuple[np.ndarray, np.ndarray]:
                ...
                if parent_id in individuals:
                    ...
                    for child_id in child_ids:
                        if child_id in individuals:
                            ...
                            if self._is_valid_parent_child_pair(parent_individual, child_individual):
                                try:
        ...
        logger.info(f"Created {len(positive_pairs)} positive and {len(negative_pairs)} negative training pairs")
        
        return np.array(positive_pairs), np.array(negative_pairs)
    
    def _is_valid_parent_child_pair(self, parent: Individual, child: Individual) -> bool:
...
def build_graph_from_gedcom(gedcom_file: str, config_path: str = "config.yaml") -> GraphData:
